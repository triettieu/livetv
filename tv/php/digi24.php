...
//$html=file_get_contents("http://www.digi24.ro/ajax/g_a_vid/101/0/9");
$html=dec($html);
$videos = explode("article_id", $html);

unset($videos[0]);
$videos = array_values($videos);

foreach($videos as $video) {
 $title=str_between($video,'article_title":"','"');
 $s_title=str_between($video,'article_short_title":"','"');
 $s_desc=str_between($video,'article_description":"','"');
 $image=str_between($video,'src":"','"');
 //http://static.digi24.ro/~cache-0~width-640~crop-1/731/03032012%20costin%20amenintari-622.jpg
 $t=explode("/",$image);
 $link=$t[0]."//".$t[2]."/".$t[4]."/".$t[5];
